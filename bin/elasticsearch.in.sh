...
if [ ! $CRATE_HEAP_SIZE ];then
    CRATE_HEAP_SIZE=32g
fi
if [ ! $CRATE_HEAP_NEWSIZE ];then
    CRATE_HEAP_NEWSIZE=2g
fi

if [ $ES_LOG_DIR ];then
    GC_LOG_DIR=$ES_LOG_DIR
else
    GC_LOG_DIR=$CRATE_HOME/logs
fi

if [ ! -e $GC_LOG_DIR ]; then
        mkdir -p $GC_LOG_DIR
fi
...
CRATE_CLASSPATH=$CRATE_HOME/lib/elasticsearch-baidu.jar:$CRATE_HOME/lib/*
...
JAVA_OPTS="$JAVA_OPTS -XX:CMSInitiatingOccupancyFraction=60"
...
JAVA_OPTS="$JAVA_OPTS -XX:MaxTenuringThreshold=7"
JAVA_OPTS="$JAVA_OPTS -XX:+UseCondCardMark"
...
JAVA_OPTS="$JAVA_OPTS -XX:NumberOfGCLogFiles=100"
JAVA_OPTS="$JAVA_OPTS -XX:GCLogFileSize=100M"
JAVA_OPTS="$JAVA_OPTS -Xloggc:$GC_LOG_DIR/es.gc.log"
