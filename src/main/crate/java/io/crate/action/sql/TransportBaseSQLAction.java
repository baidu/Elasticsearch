...
public abstract class TransportBaseSQLAction<TRequest extends SQLBaseRequest, TResponse extends SQLBaseResponse>
    ...
    public LoginUserContext getLoginUserContext(SQLBaseRequest request) {
        UserProperty userProperty = null;
        if (UserProperty.isRootLogin(request.getLoginUsername())) {
            userProperty = clusterService.state().metaData().getUserMetadata().getUserProperties().get(UserProperty.ROOT_NAME);
            UserProperty.Builder userBuilder = new UserProperty.Builder(userProperty);
            // change the tenantid to login tenantid here
            userBuilder.changeUsername(UserProperty.ROOT_NAME, UserProperty.getTenantIdFromLoginUserName(request.getLoginUsername(), clusterService.state().metaData().tenantMetadata()));
            userProperty = userBuilder.build();
        } else {
            userProperty = clusterService.state().metaData().getUserMetadata().getUserPropertyFromLoginUser(request.getLoginUsername(), clusterService.state().metaData().tenantMetadata());
        }
        return new LoginUserContext(request.getLoginUsername(), userProperty, true, false);
    }
    ...
    private void executePlan(final Analysis analysis,
        ...
        job.setParameterContext(analysis.parameterContext());
