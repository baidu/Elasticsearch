...
public class TransportExecutor implements Executor {
    ...
    public List<? extends ListenableFuture<TaskResult>> execute(Job job) {
        ...
        return execute(job.tasks(), job.getParameterContext());

    }

    private List<? extends ListenableFuture<TaskResult>> execute(Collection<? extends Task> tasks, ParameterContext parameterContext) {
        Task lastTask = null;
        assert tasks.size() > 0 : "need at least one task to execute";
        for (Task task : tasks) {
            task.setParamContext(parameterContext);
            // chaining tasks
            if (lastTask != null) {
