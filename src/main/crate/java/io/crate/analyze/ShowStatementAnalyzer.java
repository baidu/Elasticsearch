...
public class ShowStatementAnalyzer {
    ...
    // show tables should show all tables including meta table
    private String[] explicitSchemas = new String[]{"a_non_existing_schema_name"};
    ...
    public AnalyzedStatement analyze(ShowSchemas node, Analysis analysis) {
        ...
        Analysis newAnalysis = analyzer.analyze(SqlParser.createStatement(sb.toString()),
                new ParameterContext(params.toArray(), new Object[0][],
                        analysis.parameterContext().getLoginUserContext(), null));
    ...
    public AnalyzedStatement analyze(ShowTables node, Analysis analysis) {
         ...
         */
        List<String> params = new ArrayList<>();
        StringBuilder sb = new StringBuilder("SELECT concat(schema_name, '.', table_name) as table_name FROM information_schema.tables ");
        if (node.schema().isPresent()) {
            params.add(node.schema().get().toString());
