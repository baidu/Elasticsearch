...
tokens {
    ...
    ALTER_USER;
    ALTER_CLUSTER;
    ALTER_CLUSTER_ADD_NODES;
    ALTER_CLUSTER_DROP_NODES;
    ALTER_CLUSTER_DECOMMISSION_NODES;
    DROP_USER;
    DROP_CLUSTER;
    ADD_NODES;
    REMOVE_NODES;
...
showDatabasesStmt
    : SHOW DATABASES likeOrWhere? -> ^(SHOW_SCHEMAS likeOrWhere?)
    ;
...
showGrantsStmt
    : SHOW GRANTS forUsername? -> ^(SHOW_GRANTS forUsername?)
    ;
...
showClustersStmt
    : SHOW CLUSTERS clustername?-> ^(SHOW_CLUSTERS clustername?)
    ;
...
createStatement
    ...
    | USER createUserStmt -> createUserStmt
    | CLUSTER createClusterStmt -> createClusterStmt
    ;
...
createUserStmt
    : username IDENTIFIED BY password -> ^(CREATE_USER username IDENTIFIED_BY password)
    ;

createClusterStmt
    : clustername
      (WITH '(' genericProperties ')' )? -> ^(CREATE_CLUSTER clustername genericProperties?)
    ;
...
alterStatement
    ...
    | USER alterUserStmt -> alterUserStmt
    | CLUSTER alterClusterStmt -> alterClusterStmt
    ;
...
alterUserStmt
    : (username IDENTIFIED) => username IDENTIFIED BY password -> ^(RESET_PASSWORD username password)
    | (username WHITELIST) => username WHITELIST hostWhitelist -> ^(RESET_WHITELIST username hostWhitelist)
    ;

alterClusterStmt
    : (clustername SET) => clustername SET genericProperties -> ^(ALTER_CLUSTER clustername genericProperties)
    | (clustername ADD NODES) => clustername ADD NODES nodeList -> ^(ALTER_CLUSTER_ADD_NODES clustername nodeList)
    | (clustername DROP NODES) => clustername DROP NODES nodeList -> ^(ALTER_CLUSTER_DROP_NODES clustername nodeList)
    | (clustername DECOMMISSION NODES) => clustername DECOMMISSION NODES nodeList -> ^(ALTER_CLUSTER_DECOMMISSION_NODES clustername nodeList)
    ;
...
dropStatement
	...
	| USER username -> ^(DROP_USER username)
	| CLUSTER clustername -> ^(DROP_CLUSTER clustername)
	;
// END DROP STATEMENTS
...
grantStmt
    : GRANT privilege ON table TO username -> ^(GRANT_PRIVILEGE privilege table username)
    ;

revokeStmt
    : REVOKE privilege ON table FROM username -> ^(REVOKE_PRIVILEGE privilege table username)
    ;
    
migrateStmt
    : MIGRATE TABLE table TO clustername -> ^(MIGRATE_TABLE table clustername)
    ;
...
nonReserved
    ...
    | PRECEDING | RANGE | REFRESH | ROW | ROWS | SCHEMAS | DATABASES | SECOND
