...
statement returns [Statement value]
    ...
    | alterUser                 { $value = $alterUser.value; }
    | grantStmt                 { $value = $grantStmt.value; }
    | revokeStmt                { $value = $revokeStmt.value; }
    | showGrantsStmt            { $value = $showGrantsStmt.value; }
    | showUsersStmt             { $value = $showUsersStmt.value; }
    
    | createClusterStmt          { $value = $createClusterStmt.value; }
    | dropClusterStmt            { $value = $dropClusterStmt.value; }
    | showClustersStmt           { $value = $showClustersStmt.value; }
    | alterClusterStmt           { $value = $alterClusterStmt.value; }
    | migrateTableStmt          { $value = $migrateTableStmt.value; }
    ;
...
createUser returns [Statement value]
    : ^(CREATE_USER username IDENTIFIED_BY password) { $value = new CreateUser($username.value, $password.value); }
    ;

alterUser returns [Statement value]
    : ^(RESET_PASSWORD username password) { $value = new ResetPassword($username.value, $password.value); }
    | ^(RESET_WHITELIST username hostWhitelist) { $value = new ResetWhitelist($username.value, $hostWhitelist.value); }
    ;
    
grantStmt returns [Statement value]
    : ^(GRANT_PRIVILEGE privilege namedTable username) { $value = new GrantPrivilege($username.value, $privilege.value, $namedTable.value); }
    ;
    
revokeStmt returns [Statement value]
    : ^(REVOKE_PRIVILEGE privilege namedTable username) { $value = new RevokePrivilege($username.value, $privilege.value, $namedTable.value); }
    ;
    
showGrantsStmt returns [Statement value]
    : ^(SHOW_GRANTS forUsername?) { $value = new ShowGrants($forUsername.value); }
    ;

showUsersStmt returns [Statement value]
    : SHOW_USERS { $value = new ShowGrants(null); }
    ;

privilege returns [String value]
    : r=READ_ONLY { $value = $r.text.toLowerCase(Locale.ENGLISH); }
    | w=READ_WRITE { $value = $w.text.toLowerCase(Locale.ENGLISH); }
    ; 

forUsername returns [String value]
    : ^(FOR username) { $value = $username.value; }
    ;
...
password returns [String value]
    : ident { $value = $ident.value; }
    ;
    
hostWhitelist returns [String value]
    : q=QUOTED_IDENT { $value = $q.text; }
    ;
    
createClusterStmt returns [Statement value]
    :  ^(CREATE_CLUSTER clustername genericProperties?) { $value = new CreateTenant($clustername.value, $genericProperties.value);} 
    ;

dropClusterStmt returns [Statement value]
    : ^(DROP_CLUSTER clustername) { $value = new DropTenant($clustername.value); }
    ;
    
showClustersStmt returns [Statement value]
    : ^(SHOW_CLUSTERS clustername?) { $value = new ShowTenants($clustername.value); }
    ;
    
alterClusterStmt returns [Statement value]
    : ^(ALTER_CLUSTER clustername genericProperties) { $value = new AlterTenantProperty($clustername.value, $genericProperties.value); }
    | ^(ALTER_CLUSTER_ADD_NODES clustername nodeList) { $value = new AlterTenantModifyNodes($clustername.value, $nodeList.value, TenantModificationOperation.ADD_NODES); }
    | ^(ALTER_CLUSTER_DROP_NODES clustername nodeList) { $value = new AlterTenantModifyNodes($clustername.value, $nodeList.value, TenantModificationOperation.DROP_NODES); }
    | ^(ALTER_CLUSTER_DECOMMISSION_NODES clustername nodeList) { $value = new AlterTenantModifyNodes($clustername.value, $nodeList.value, TenantModificationOperation.DECOMMISSION_NODES); }
    ;
    
migrateTableStmt returns [Statement value]
    : ^(MIGRATE_TABLE namedTable clustername) { $value = new MigrateTable($namedTable.value, $clustername.value); }
    ;

clustername returns [String value]
    : ident { $value = $ident.value; }
    ;
    
nodeList returns [String value]
    : ident { $value = $ident.value; }
    ;
...
dropUser returns [Statement value]
    : ^(DROP_USER username) { $value = new DropUser($username.value); }
    ;
