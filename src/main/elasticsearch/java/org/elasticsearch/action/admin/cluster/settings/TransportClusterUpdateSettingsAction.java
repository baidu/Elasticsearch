        ...
        clusterService.submitStateUpdateTask("cluster_update_settings",
            ...
            public ClusterState execute(final ClusterState currentState) {
                ...
                if (!Strings.isNullOrEmpty(tenantName)) {
                    if (!currentState.metaData().tenantMetadata().tenantExist(tenantName)) {
                        throw new ElasticsearchException("tenant [" + tenantName + "] not exist");
                    }
                }
                ...
                for (Map.Entry<String, String> entry : request.transientSettings().getAsMap().entrySet()) {
                    if (dynamicSettings.isDynamicOrLoggingSetting(entry.getKey())) {
                        ...
                        if (error == null) {
                            transientSettings.put(tenantName, entry.getKey(), entry.getValue());
                            transientUpdates.put(tenantName, entry.getKey(), entry.getValue());
                            changed = true;
                        } else {
                ...
                for (Map.Entry<String, String> entry : request.persistentSettings().getAsMap().entrySet()) {
                    if (dynamicSettings.isDynamicOrLoggingSetting(entry.getKey())) {
                        String error = dynamicSettings.validateDynamicSetting(entry.getKey(), entry.getValue(), clusterService.state());
                        if (error == null) {
                            persistentSettings.put(tenantName, entry.getKey(), entry.getValue());
                            persistentUpdates.put(tenantName, entry.getKey(), entry.getValue());
                            changed = true;
                        } else {
                ...
                for (String settingName : request.transientSettingsToRemove()) {
                    if (dynamicSettings.hasDynamicSetting(settingName)) {
                        transientSettings.remove(tenantName, settingName);
                        changed = true;
                    } else {
                ...
                for (String settingName : request.persistentSettingsToRemove()) {
                    if (dynamicSettings.hasDynamicSetting(settingName)) {
                        persistentSettings.remove(tenantName, settingName);
                        changed = true;
                    } else {
