...
public class IndexRequest extends ReplicationRequest<IndexRequest> implements DocumentRequest<IndexRequest> {
    ...
    public void process(MetaData metaData, @Nullable MappingMetaData mappingMd, boolean allowIdGeneration, String concreteIndex) {
        ...
        boolean hasParsedVersion = false;
        boolean shouldParseVersion = false;
        // extract values if needed
        if (mappingMd != null) {
            MappingMetaData.ParseContext parseContext = mappingMd.createParseContext(id, routing, timestamp, version);
            ...
            if (parseContext.shouldParse()) {
                ...
                try {
                    ...
                    if (parseContext.shouldParseVersion()) {
                        shouldParseVersion = true;
                        String parsedVersion = parseContext.version();
                        if (parsedVersion != null) {
                            versionType = mappingMd.version().versionType();
                            version = Long.parseLong(parsedVersion);
                            hasParsedVersion = true;
                        }
                    }
                } catch (MapperParsingException e) {
                    throw e;
                } catch (Exception e) {
                    throw new ElasticsearchParseException("failed to parse doc to extract routing/timestamp/id/version", e);
                } finally {
                    if (parser != null) {
                        parser.close();
                    }
        ...
        if (shouldParseVersion && !hasParsedVersion) {
            if (mappingMd != null && mappingMd.version() != null && mappingMd.version().path() != null 
                    && mappingMd.version().path().equals(TimestampFieldMapper.NAME) && timestamp != null) {
                versionType = mappingMd.version().versionType();
                version = Long.parseLong(timestamp);
            } else {
                throw new IllegalArgumentException("version is required by mapping");
            }
        }
    }
