                    ...
                    if (!renamedIndices.isEmpty()) {
                        ...
                        for (Map.Entry<String, String> indexEntry : renamedIndices.entrySet()) {
                            ...
                            if (currentIndexMetaData == null) {
                                ...
                                if (IndexMetaData.isIndexUsingDLEngine(updatedIndexMetaData.getSettings())) {
                                    for (int i = 0; i < updatedIndexMetaData.getNumberOfShards(); ++i) {
                                        logger.info("try to create a distributed log in dl index [{}], uuid [{}], shardid [{}]", 
                                                updatedIndexMetaData.getIndex(), updatedIndexMetaData.getIndexUUID(), i);
                                        DistributedTranslog.createLog(settings, updatedIndexMetaData.getIndex(), 
                                                updatedIndexMetaData.getIndexUUID(), i, currentState.nodes().getLocalNodeId());
                                    }
                                }
                                ...
                                if (IndexMetaData.isIndexUsingDLEngine(currentIndexMetaData.getSettings()) || IndexMetaData.isIndexUsingDLEngine(snapshotIndexMetaData.getSettings())) {
                                    throw new SnapshotRestoreException(snapshotId, "cannot restore index [" + renamedIndex + "], DL based index engine must delete the original index first");
                                }
