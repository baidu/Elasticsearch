    ...
    static void finalizeSettings(Settings.Builder output, Terminal terminal, Path configDir) {
        ...
        if (output.get("name") == null) {
            ...
            if (name == null || name.isEmpty()) {
                if (output.get("tcp.port") != null && !output.get("tcp.port").isEmpty()) {
                    try {
                        // if user not set name successfully, we use hostname + tcp.port firstly
                        InetAddress addr;
                        addr = InetAddress.getLocalHost();
                        name = addr.getHostName() + ":" + output.get("tcp.port");
