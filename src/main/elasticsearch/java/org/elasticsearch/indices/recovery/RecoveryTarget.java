...
public class RecoveryTarget extends AbstractComponent {
    ...
    public static class Actions {
        ...
        public static final String CHECK_TRANSLOG_OFFSET = "internal:index/shard/recovery/check_translog_offset";
    ...
    protected void retryRecovery(final RecoveryStatus recoveryStatus, final String reason, TimeValue retryAfter, final StartRecoveryRequest currentRequest) {
        ...
        try {
            ...
            // wait 1000ms to retry again, or there will be too many retry logs
            Thread.sleep(1000);
        } catch (Throwable e) {
    ...
    private void doRecovery(final RecoveryStatus recoveryStatus) {
        ...
        try {
            logger.debug("[{}][{}] starting recovery from {}", request.shardId().index().name(), request.shardId().id(), request.sourceNode());
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("[{}][{}] Got exception on recovery", e, request.shardId().index().name(), request.shardId().id());
            }
    ...
    class PrepareForTranslogOperationsRequestHandler extends TransportRequestHandler<RecoveryPrepareForTranslogOperationsRequest> {
        ...
        public void messageReceived(RecoveryPrepareForTranslogOperationsRequest request, TransportChannel channel) throws Exception {
            logger.debug("begin to prepare to recovery from translog");
    ...
    class CheckTranslogOffsetRequestHandler extends TransportRequestHandler<RecoveryCheckTranslogOffsetRequest> {

        @Override
        public void messageReceived(RecoveryCheckTranslogOffsetRequest request, TransportChannel channel) throws Exception {
            logger.debug("begin to prepare to recovery from translog");
            long offset = 0;
            try (RecoveriesCollection.StatusRef statusRef = onGoingRecoveries.getStatusSafe(request.recoveryId(), request.shardId())) {
                final RecoveryStatus recoveryStatus = statusRef.status();
                offset = recoveryStatus.indexShard().getTranslogOffset();
            }
            channel.sendResponse(new RecoveryCheckTranslogOffsetResponse(offset));
        }
    }
