...
public class HierarchyCircuitBreakerService extends CircuitBreakerService {
    ...
    public void checkParentLimit(String label) throws CircuitBreakingException {
        ...
        if (totalUsed > parentLimit) {
            String breakerDetails = "";
            for (CircuitBreaker breaker : this.breakers.values()) {
                breakerDetails += String.format("breaker detail name: %s, limit %d, used %d, trip count %d]", 
                        breaker.getName(), breaker.getLimit(), breaker.getUsed(),  breaker.getTrippedCount());
            }
            ...
            throw new CircuitBreakingException("[parent] Data too large, data for [" +
                    ...
                    "], currently used " + totalUsed + ", parent limit " + parentLimit + "/n" + 
                    breakerDetails,
