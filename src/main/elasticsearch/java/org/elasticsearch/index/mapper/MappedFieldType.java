...
public abstract class MappedFieldType extends FieldType {
     ...
     */
    public boolean checkCompatibility(MappedFieldType other, List<String> conflicts, boolean strict, boolean reindex) {
        checkTypeName(other);
        ...
        // if index change from true to false, no need cheak and no need reindex
        ...
        // if index change from true to false, and reindex flag is false, no need reindex
        if (indexed == true && mergeWithIndexed == false && reindex == false && !conflicts.isEmpty()) {
            needReindex = false;
            conflicts.clear();
        } else if (reindex == true && !conflicts.isEmpty()) {
