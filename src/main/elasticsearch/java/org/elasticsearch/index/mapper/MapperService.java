...
public class MapperService extends AbstractIndexComponent implements Closeable {
    ...
    private static ObjectHashSet<String> META_FIELDS = ObjectHashSet.from(
            ...
            "_size", "_timestamp", "_ttl", "_version"
    );
    ...
    public DocumentMapper merge(String type, CompressedXContent mappingSource, MergeReason reason, boolean updateAllTypes) {
        return merge(type, mappingSource, reason, updateAllTypes, false);
    }
    
    public DocumentMapper merge(String type, CompressedXContent mappingSource, MergeReason reason, boolean updateAllTypes, boolean reindex) {
        return merge(type, mappingSource, reason, updateAllTypes, reindex, -1L);
    }
    ...
    private synchronized DocumentMapper merge(DocumentMapper mapper, MergeReason reason, boolean updateAllTypes, boolean reindex, long mappingVersion) {
        ...
        FieldTypeLookup fieldTypes = this.fieldTypes.copyAndAddAll(newMapper.type(), fieldMappers, updateAllTypes, reindex);
        ...
        // change mappers version
        // it means this is a update mapping request
        if (mappingVersion >= 0) {
            mappers.get(newMapper.type()).updateMappingVersion(mappingVersion);
        }
        if (fieldTypes.needReindex()) {
            newMapper.setReindex(true);
        }
        // 4. commit the change
