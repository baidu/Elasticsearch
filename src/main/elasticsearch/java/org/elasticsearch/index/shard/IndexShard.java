...
public class IndexShard extends AbstractIndexShardComponent {
    ...
    protected final InternalIndicesLifecycle indicesLifecycle;
    ...
    protected EnumSet<IndexShardState> readAllowedStates = EnumSet.of(IndexShardState.STARTED, IndexShardState.RELOCATED, IndexShardState
            .POST_RECOVERY);
    ...
    public static final String INDEX_REINDEX_BATCH_SIZE = "index.reindex_batch_size";
    public static final String INDEX_REINDEX_INTERVAL= "index.reindex_interval";

    private int reindexBatchSize;
    private TimeValue reindexInterval;
    private SearchService shardSearchService;
     ...
     */
    protected IndexShardState changeState(IndexShardState newState, String reason) {
    ...
    public void create(Engine.Create create) {
        if (create.origin() == Operation.Origin.PRIMARY) {
            clusterService.state().blocks().tenantBlockedRaiseException(ClusterBlockLevel.WRITE, 
                    clusterService.state().metaData().index(this.shardId.getIndex()).indexOwnerTenantId());
        }
    ...
    private void startScheduledTasksIfNeeded() {
        if (refreshInterval.millis() > 0) {
            // should not use same thread pool, use generic
            refreshScheduledFuture = threadPool.schedule(refreshInterval, ThreadPool.Names.GENERIC, new EngineRefresher());
            logger.debug("scheduling refresher every {}", refreshInterval);
        } else {
            logger.debug("scheduled refresher disabled");
        }
        if (reindexInterval.millis() > 0) {
            threadPool.schedule(reindexInterval, ThreadPool.Names.REINDEX, new EngineReindexer());
            logger.debug("scheduling reindex check every {}", refreshInterval);
        } else {
            logger.debug("scheduled reindex check disabled");
        }
    }
    ...
    private class ApplyRefreshSettings implements IndexSettingsService.Listener {
        ...
        public void onRefreshSettings(Settings settings) {
            ...
            synchronized (mutex) {
                ...
                TimeValue reindexInterval = settings.getAsTime(INDEX_REINDEX_INTERVAL, IndexShard.this.reindexInterval);
                if (reindexInterval.micros() > 0) { // only care index interval > 0
                    logger.info("updating reindex_interval from [{}] to [{}]", IndexShard.this.reindexInterval, reindexInterval);
                    IndexShard.this.reindexInterval = reindexInterval;
                }
                
                IndexShard.this.reindexBatchSize = settings.getAsInt(INDEX_REINDEX_BATCH_SIZE, IndexShard.this.reindexBatchSize);
    ...
    public ReindexStats reindexStats() {
        long needReindexDocNum = 0;
        long totalDocNum = 0;
        long rate = 0;
        for (final String type : indexService.mapperService().types()) {
            long currentMappingVersion = clusterService.state().metaData().index(shardId.getIndex())
                    .mapping(type).mappingVersion();
            
            QueryFetchSearchResult hits = getShardNeedReindexDoc(type, currentMappingVersion, 1);
            if (hits.queryResult().topDocs().totalHits > 0) {
                needReindexDocNum += hits.queryResult().topDocs().totalHits;
                long typeDocNum = getShardDocNumByType(type);
                totalDocNum += typeDocNum;
            }
        }
        ...
        if (totalDocNum == 0) {
            ...
            rate = (totalDocNum - needReindexDocNum) * 100 / totalDocNum;
        }

        return new ReindexStats(rate);
    }

    public static final String MAPPING_VERSION_QUERY_PATTERN = "{ query: {\"range\": {\"_mapping_version\":{\"gte\":0, \"lt\": %d}}}, \"fielddata_fields\": [ \"_mapping_version\", \"_version\",\"_timestamp\", \"_parent\"], \"size\": %d}";
    public static final String DOCNUM_QUERY_PATTERN = "{ query: {\"match_all\": {}}, \"size\": %d}";
    public QueryFetchSearchResult getShardNeedReindexDoc(String type, long currentMappingVersion, int size) {
        String query = String.format(MAPPING_VERSION_QUERY_PATTERN, currentMappingVersion, size);
    ...
    class EngineReindexer implements Runnable {
        @Override
        public void run() {
            try {
                if (reindexBatchSize < 1) {
                    return;
                }
                for (final String type : indexService.mapperService().types()) {
                    final long currentMappingVersion = clusterService.state().metaData().index(shardId.getIndex())
                            .mapping(type).mappingVersion();
                    final QueryFetchSearchResult hits = getShardNeedReindexDoc(type, currentMappingVersion, reindexBatchSize);
                    if (hits.queryResult().topDocs().totalHits > 0) {
                        try {
                            reindex(hits, shardId.getIndex(), type);
                        } catch (VersionConflictEngineException ve) {
                            logger.debug("EngineReindexer version conflict: ", ve);
                        } catch (Throwable t) {
                            logger.error("EngineReindexer run error: ", t);
                        }
                     }
                 }
             } catch (Throwable t) {
                 logger.error("errors while reindex", t);
             } finally {
                 synchronized (mutex) {
                     if (state != IndexShardState.CLOSED && refreshInterval.millis() > 0) {
                         threadPool.schedule(reindexInterval, ThreadPool.Names.REINDEX, this);
                     }
                 }
             }
         }
