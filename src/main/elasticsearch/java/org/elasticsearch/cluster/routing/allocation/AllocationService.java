...
public class AllocationService extends AbstractComponent {
    ...
    public RoutingAllocation.Result applyFailedShard(ClusterState clusterState, ShardRouting failedShard) {
        return applyFailedShards(clusterState, Collections.singletonList(new FailedRerouteAllocation.FailedShard(failedShard, null, null, null)));
    }
    ...
    public RoutingAllocation.Result applyFailedShards(ClusterState clusterState, List<FailedRerouteAllocation.FailedShard> failedShards) {
        ...
        for (FailedRerouteAllocation.FailedShard failedShard : failedShards) {
            // for failures find by the node itself, should not delay
            if (failedShard.finderNodeId != null && failedShard.finderNodeId.equalsIgnoreCase(failedShard.shard.currentNodeId())) {
                ...
                changed |= applyFailedShard(allocation, failedShard.shard, false, new UnassignedInfo(UnassignedInfo.Reason.PRIMARY_REPORT_FAILURE, failedShard.message, failedShard.failure,
                        System.nanoTime(), System.currentTimeMillis()));
            }
        }
        if (!changed) {
    ...
    private boolean deassociateDeadNodes(RoutingAllocation allocation) {
        ...
        for (RoutingNodes.RoutingNodesIterator it = allocation.routingNodes().nodes(); it.hasNext(); ) {
            ...
            for (ShardRouting shardRouting : node.copyShards()) {
                // could not print detail node info here because the node is removed from metadata and could not find it any more only id
