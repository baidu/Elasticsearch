...
public class OperationRouting extends AbstractComponent {
    ...
    public ShardIterator getShards(ClusterState clusterState, String index, String type, String id, @Nullable String routing, @Nullable String preference) {
        return preferenceActiveShardIterator(clusterState, shards(clusterState, index, type, id, routing), clusterState.nodes().localNodeId(), clusterState.nodes(), preference);
    }
    ...
    public ShardIterator getShards(ClusterState clusterState, String index, int shardId, @Nullable String preference) {
        ...
        return preferenceActiveShardIterator(clusterState, indexShard, clusterState.nodes().localNodeId(), clusterState.nodes(), preference);
    }
    ...
    public GroupShardsIterator searchShards(ClusterState clusterState, String[] concreteIndices, @Nullable Map<String, Set<String>> routing, @Nullable String preference) {
        ...
        for (IndexShardRoutingTable shard : shards) {
            ShardIterator iterator = preferenceActiveShardIterator(clusterState, shard, clusterState.nodes().localNodeId(), clusterState.nodes(), preference);
            if (iterator != null) {
                set.add(iterator);
            }
        }
        ...
        for (String index : concreteIndices) {
            ...
            if (effectiveRouting != null) {
                for (String r : effectiveRouting) {
                    int shardId = generateShardId(clusterState, index, null, null, r);
    ...
    private ShardIterator preferenceActiveShardIterator(ClusterState state, IndexShardRoutingTable indexShard, String localNodeId, DiscoveryNodes nodes, @Nullable String preference) {
        if (preference == null || preference.isEmpty()) {
            // if user not specified preference and local engine is dl engine, then primary must be used
            IndexMetaData indexMeta = state.getMetaData().index(indexShard.shardId.index().name());
            if (IndexMetaData.isIndexUsingDLEngine(indexMeta.getSettings())) {
    ...
    public ShardId shardId(ClusterState clusterState, String index, String type, String id, @Nullable String routing) {
        return new ShardId(index, generateShardId(clusterState, index, type, id, routing));
    }
