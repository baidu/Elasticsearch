    ...
    public RerouteExplanation execute(RoutingAllocation allocation, boolean explain) {
        ...
        for (RoutingNodes.UnassignedShards.UnassignedIterator it = routingNodes.unassigned().iterator(); it.hasNext(); ) {
            ...
            if (unassigned.primary()) {
                IndexMetaData indexMetaData = allocation.metaData().index(unassigned.getIndex());
                // if this is a primary allocation, then delete the log and create a new one
                try {
                    Settings settings = allocation.deciders().getSettings();
                    DistributedTranslog.deleteLog(settings, indexMetaData.getIndexUUID(), unassigned.getIndex(), unassigned.getId(), allocation.nodes().localNodeId());
                    DistributedTranslog.createLog(settings, unassigned.getIndex(), indexMetaData.getIndexUUID(), unassigned.getId(), allocation.nodes().localNodeId());
                } catch (Exception e) {
                    throw new ElasticsearchException("errors while create or delete log", e);
                }
            }
