...
public class ClusterState implements ToXContent, Diffable<ClusterState> {
    ...
    public XContentBuilder toXContent(XContentBuilder builder, Params params) throws IOException {
        ...
        if (metrics.contains(Metric.NODES)) {
            ...
            builder.endObject();
            builder.startObject("inactive");
            for (ObjectObjectCursor<String, DiscoveryNode> node : nodes.deadNodes()) {
        ...
        if (metrics.contains(Metric.METADATA)) {
            ...
            UserMetadata.Builder.toXContent(metaData().getUserMetadata(), builder, params);
            ...
            for (IndexMetaData indexMetaData : metaData()) {
                ...
                for (ObjectObjectCursor<String, MappingMetaData> cursor : indexMetaData.getMappings()) {
                    ...
                    if (mapping.containsKey("_meta")) {
                        ((Map<String, Object>)mapping.get("_meta")).put(MappingMetaData.MAPPING_VERSION, cursor.value.mappingVersion());
                    } else {
                        HashMap<String, Object> mappingMeta = new HashMap<>();
                        mappingMeta.put(MappingMetaData.MAPPING_VERSION, cursor.value.mappingVersion());
                ...
                builder.endArray();
                long tenantId = indexMetaData.indexOwnerTenantId();
                String tenantName = tenantId == TenantProperty.ROOT_TENANT_ID ? TenantProperty.ROOT_TENANT_NAME : metaData.tenantMetadata().tenant(tenantId).tenantName();
                builder.field("tenant", tenantName);
                builder.endObject();
            }
            builder.endObject();
    ...
    public class ClusterStateDiff implements Diff<ClusterState> {
        ...
        public ClusterStateDiff(ClusterState before, ClusterState after) {
            ...
            blocks = after.blocks.diff(null); // not use before.blocks, force use full blocks
        ...
        public ClusterState applyIgnoreUUID(ClusterState state) {
            Builder builder = new Builder(clusterName);
            if (toUuid.equals(state.stateUUID)) {
                // no need to read the rest - cluster state didn't change
                return state;
