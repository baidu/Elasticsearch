    ...
    private boolean refreshIndexMapping(IndexService indexService, IndexMetaData.Builder builder) {
        ...
        try {
            ...
            if (updatedTypes.isEmpty() == false) {
                ...
                dirty = true;
                for (DocumentMapper mapper : indexService.mapperService().docMappers(true)) {
                    long version = builder.mapping(mapper.type()).mappingVersion();
                    builder.putMapping(new MappingMetaData(mapper, version));
                }
            }
        } catch (Throwable t) {
    ...
    class PutMappingExecutor extends ClusterStateTaskExecutor<PutMappingClusterStateUpdateRequest> {
                    ...
                    for (String index : request.indices()) {
                        ...
                        if (indexMetaData != null  && indicesService.hasIndex(index) == false) {
                            ...
                            for (ObjectCursor<MappingMetaData> mapping : indexMetaData.getMappings().values()) {
                                indexService.mapperService().merge(mapping.value.type(), mapping.value.source(), MapperService.MergeReason.MAPPING_RECOVERY, request.updateAllTypes(), request.reindex());
                            }
                        }
                    }
                }
        ...
        private ClusterState applyRequest(ClusterState currentState, PutMappingClusterStateUpdateRequest request) throws IOException {
            ...
            for (String index : request.indices()) {
                ...
                DocumentMapper mergedMapper = indexService.mapperService().merge(mappingType, mappingUpdateSource, MapperService.MergeReason.MAPPING_UPDATE, request.updateAllTypes(), request.reindex());
                ...
                for (DocumentMapper mapper : indexService.mapperService().docMappers(true)) {
                    long newMappingVersion = 1;
                    MappingMetaData oldMappingMetaData = indexMetaData.mapping(mapper.type());
                    if (oldMappingMetaData != null) {
                        newMappingVersion = oldMappingMetaData.mappingVersion();
                    }
                    if (mergedMapper.needReindex() && mergedMapper.type().equals(mapper.type())) {
                        ++ newMappingVersion;
                    }
                    mapper.updateMappingVersion(newMappingVersion);
                    MappingMetaData newMappingMetaData = new MappingMetaData(mapper.mappingSource());
                    newMappingMetaData.setMappingVersion(mapper.mappingVersion());
                    indexMetaDataBuilder.putMapping(newMappingMetaData);
                }
