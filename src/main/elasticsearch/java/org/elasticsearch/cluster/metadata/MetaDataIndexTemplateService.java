...
public class MetaDataIndexTemplateService extends AbstractComponent {
    ...
    public void removeTemplates(final RemoveRequest request, final RemoveListener listener) {
        long currentTenantId = request.getLoginUserContext().tenantId();
        if (!UserProperty.ROOT_NAME.equalsIgnoreCase(UserProperty.getUsernameWithoutTenantFromFullUsername(request.getLoginUserContext().fullUsername()))) {
            long ownerTenantId = clusterService.state().metaData().templates().get(request.name).templateOwnerTenantId();
            if (ownerTenantId != currentTenantId) {
                throw new ElasticsearchException("current operation tenant [" + currentTenantId + "] is not the owner tenant for index [" + request.name + "]");
            }
        }
        clusterService.submitStateUpdateTask("remove-index-template [" + request.name + "]", new ClusterStateUpdateTask(Priority.URGENT) {
            ...
            public ClusterState execute(ClusterState currentState) {
                ...
                for (ObjectCursor<IndexTemplateMetaData> cursor : currentState.metaData().templates().values()) {
                    String templateName = cursor.value.name();
                    // drop template owner or root
                    if (Regex.simpleMatch(request.name, templateName) && (currentTenantId == TenantProperty.ROOT_TENANT_ID || currentTenantId == cursor.value.templateOwnerTenantId())) {
    ...
    public void putTemplate(final PutRequest request, final PutListener listener) {
        ...
        try {
            ...
            long tenantId = request.getLoginUserContext().tenantId();
            templateBuilder.templateOwnerTenantId(tenantId);
    ...
    public static class PutRequest extends LoginUserContextHeader {
        final String name;
        final String cause;
        boolean create;
        int order;
        ...
        public PutRequest(String cause, String name, LoginUserContext loginUserContext) {
            super(loginUserContext);
            this.cause = cause;
            this.name = name;
        }
    ...
    public static class RemoveRequest extends LoginUserContextHeader {
        final String name;
        TimeValue masterTimeout = MasterNodeRequest.DEFAULT_MASTER_NODE_TIMEOUT;

        public RemoveRequest(String name, LoginUserContext userContext) {
            super(userContext);
            this.name = name;
        }
