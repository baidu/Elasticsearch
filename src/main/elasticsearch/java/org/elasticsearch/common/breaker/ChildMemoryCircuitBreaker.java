...
public class ChildMemoryCircuitBreaker implements CircuitBreaker {
    ...
    public double addEstimateBytesAndMaybeBreak(long bytes, String label) throws CircuitBreakingException {
        ...
        try {
            ...
            logger.warn("try to add [{}] bytes to breaker [{}], but exceed limit, remove it from breaker now, current used [{}]", bytes, name, used);
            throw e;
        }
        return newUsed;
    }
    ...
    public long addWithoutBreaking(long bytes) {
        long previousUsed = used.get();
        long u = used.addAndGet(bytes);
        // if allocated too much mem , then print stack trace
        if (bytes > 10000000 || u > memoryBytesLimit) {
            Exception e = new ElasticsearchException("try to adjust a too large memory, size is: " + bytes + ", previous used memory is: " + previousUsed);
            logger.warn("something wrong with adjust", e);
        }
        if (logger.isTraceEnabled()) {
