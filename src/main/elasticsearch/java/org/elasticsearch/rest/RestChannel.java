...
public abstract class RestChannel {
    ...
    private static final DeprecationLogger DEPRECATION_LOGGER = new DeprecationLogger(Loggers.getLogger(RestChannel.class));
    
    private final AuditLog auditLog = AuditService.getAuditLog();
    ...
    public void sendResponse(RestResponse response) {
        String requestBody = request.method().name() + "\t" + request.uri() + "\t" + request.content().toUtf8();
        long endTime = System.currentTimeMillis();
        long startTime = request.getFromContext(RestRequest.REQUEST_START_TIME);
        LoginUserContext user = request.getHeader(LoginUserContext.USER_INFO_KEY);
        String message = "Rest Status " + response.status().toString();
        if (user != null) {
            auditLog.log(startTime, user.loginUsername(), user.sourceAddrs(), requestBody, endTime - startTime, message);
        } else {
            auditLog.log(startTime, "", "", requestBody, endTime - startTime, message);
        }
        doSendResponse(response);
    }
    
    public abstract void doSendResponse(RestResponse response);
}
